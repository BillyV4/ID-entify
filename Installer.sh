...
echo -e "$OKGREEN V 0.2 $RESET"
echo -e "$OKGREEN  $RESET"


INSTALL_DIR=/usr/share/ID-entify

echo -e "$OKGREEN THIS SCRIPT WILL BE INSTALLED. PRESS A KEY TO CONTINUE $RESET"
read answer 
...
problem=$(dpkg -s fierce | grep installed)
 echo Checking for fierce : $problem
 if [ "" == "$problem" ]; then
...
dpkg --get-selections | grep fierce
